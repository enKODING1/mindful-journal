'use client';

import Container from '@/components/ui/atom/Container';
import Avatar from '@/components/ui/atom/Avatar';
import Button from '@/components/ui/atom/Button';
import { useAuth } from '@/hooks';
import { useWebViewDetect } from '@/hooks/useWebViewDetect';
import { ExternalLink, Copy } from 'lucide-react';

export default function AuthPage() {
    const { loginWithGoogle, loading, error } = useAuth();
    const { isWebView, isAndroid, openInExternalBrowser } = useWebViewDetect();

    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4">
            <Container
                variant="base-100"
                padding="xl"
                gap="lg"
                rounded="2xl"
                centered={true}
                className="max-w-md w-full"
            >
                <div className="flex flex-col items-center gap-4 text-center">
                    <Avatar size="lg" src="/icon_wave.svg" />
                    <div className="flex flex-col gap-2">
                        <h1 className="text-3xl font-bold text-primary">마음챙김 일기</h1>
                        <p className="text-base-content/70">
                            매일의 생각과 감정을 기록하며
                            <br />
                            마음의 평화를 찾아보세요
                        </p>
                    </div>
                </div>

                {error && (
                    <div className="bg-error/10 text-error p-3 rounded-lg text-sm text-center">
                        {error}
                    </div>
                )}

                {isWebView ? (
                    <div className="flex flex-col gap-3 w-full">
                        <div className="bg-base-200 border border-base-300 p-4 rounded-xl text-sm text-base-content/70 text-center">
                            Google 로그인은 인앱 브라우저에서 지원되지 않아요.
                            <br />
                            외부 브라우저에서 열어주세요.
                        </div>
                        <Button
                            type="button"
                            variant="primary"
                            size="lg"
                            className="w-full"
                            onClick={openInExternalBrowser}
                        >
                            {isAndroid ? (
                                <>
                                    <ExternalLink className="w-5 h-5 mr-2" />
                                    Chrome에서 열기
                                </>
                            ) : (
                                <>
                                    <Copy className="w-5 h-5 mr-2" />
                                    링크 복사 후 Safari에서 열기
                                </>
                            )}
                        </Button>
                    </div>
                ) : (
                    <Button
                        type="button"
                        variant="primary"
                        size="lg"
                        className="w-full"
                        onClick={loginWithGoogle}
                        disabled={loading}
                    >
                        <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
                            <path
                                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                                fill="#4285F4"
                            />
                            <path
                                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                                fill="#34A853"
                            />
                            <path
                                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                                fill="#FBBC05"
                            />
                            <path
                                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                                fill="#EA4335"
                            />
                        </svg>
                        {loading ? '로그인 중...' : 'Google로 시작하기'}
                    </Button>
                )}
            </Container>
        </div>
    );
}
